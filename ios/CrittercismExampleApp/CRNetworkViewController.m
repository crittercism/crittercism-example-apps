...
#define kWebViewSection 3
#define kTextViewSection 4
...
@property (nonatomic, strong) NSMutableString *textViewContents;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == kConnectionSection || indexPath.section == kProtocolSection) {
        ...
        cell.textLabel.text = _cellNames[indexPath.section][indexPath.row];
        cell.accessoryType = UITableViewCellAccessoryNone;

        if ([self isCellSelectedAtIndexPath:indexPath]) {
            cell.accessoryType = UITableViewCellAccessoryCheckmark;
        }
    } else if (indexPath.section == kWebViewSection) {
        cell = [tableView dequeueReusableCellWithIdentifier:@"UIWebViewCell"];
        cell.textLabel.text = _cellNames[indexPath.section][indexPath.row];
        ...
        buttonCell = (id)cell;
        [[buttonCell aButton] setTitle:_cellNames[indexPath.section][indexPath.row][0]
                              forState:UIControlStateNormal];
        [[buttonCell aButton] addTarget:self
                                 action:@selector(hitButton:)
                       forControlEvents:UIControlEventTouchUpInside];

        [[buttonCell bButton] setTitle:_cellNames[indexPath.section][indexPath.row][1]
                              forState:UIControlStateNormal];
        [[buttonCell bButton] addTarget:self
                                 action:@selector(hitButton:)
                       forControlEvents:UIControlEventTouchUpInside];

        [[buttonCell cButton] setTitle:_cellNames[indexPath.section][indexPath.row][2]
                              forState:UIControlStateNormal];
...
- (NSString *) tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section
{
    if(section == kConnectionSection) {
        ...
        return @"Punch It:";
    } else if (section == kWebViewSection) {
        return @"Web Views:";
...
- (void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section != kPunchItSection) {
        _selectedRows[indexPath.section] = @(indexPath.row);
    }
...
{
    [[GlobalLog sharedLog] logActionString:[NSString stringWithFormat:@"[Network]: %@", command]];
...
- (BOOL)isCellSelectedAtIndexPath:(NSIndexPath *)path {
    return (path.section != kPunchItSection) && (path.section != kTextViewSection) &&
        ([_selectedRows[path.section] isEqual:@(path.row)]);
}
